...
JHU_C %>% filter(is.na(province)) # Confine to country level data

str(Population)
  paste(Population$location, sep='\t')
  Population <- Population %>% filter(!is.na(iso_code)) # No iso code for International
...
## Check
JHUNames %>% anti_join(Population, by = c( 'location' = 'location')) # Two ships
