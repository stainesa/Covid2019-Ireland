...
To prepare models for coronvairus incidence, and short term predictions, in Ireland
...
# Prepare a geenral data set of confirmed cases
...
pred.frame=tibble(Day = seq(1:38)) # 7 days ahead.
```
...
```{r lm}
Ireland <- 
model.lm <- lm(data=Prepared %>% filter(Country.Region == 'Ireland') %>% filter(Sum.Cases >0),
               log10(Sum.Cases) ~ Day)
