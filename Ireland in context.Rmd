...
```{r}
set.seed(6564)
Max_date = max(JHU_C_iso$date)
Min_date = min(JHU_C_iso$date)
Span = as.numeric(Max_date - Min_date)

#Set of dates to play with
Latest_date <- JHU_C_iso %>%
  group_by(country, type) %>% # Type within country
  mutate(weight = 1 - 2*as.numeric(Max_date - date)/Span) %>% #weight more recent ranks more heavily
...
ggContextRatesShort <- ggContextRates +
  scale_x_date(limits = c(as_date('2020-09-01'), as_date('2021-01-15'))) +
  geom_label(aes(x = as_date('2021-09-01') + Span*5/100,
                 y = max(rate)/2),
             colour = 'blue', fill = 'lightblue',
             label='@isagcovid19') 
